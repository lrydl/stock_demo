...
public class MergeQueue {
   ...
   private static int threshold = 3;//空轮询若干次撤销异步线程，当tps下降如何处理？ 当tps下降时，在controller层走的是单个扣减逻辑，即请求不会打到MergeQueue,所以只需要检测空轮询即可
    ...
    public static class AsyncThread extends Thread{
        ...
        public boolean offer(RequestPromise requestPromise, long time, TimeUnit unit) throws InterruptedException {
            if(!isRunning()){
                return false;
            }
        ...
        public void run() {
            while (running) {
                ...
                for (int i = 0; i < batchSize; i++) {
                    RequestPromise requestPromise = queue.poll();
        ...
        private void degrade(List<RequestPromise> list){
